...
def record_audio(ask = False):
    ...
    with sr.Microphone() as source:
        ...
        try:
            ...
            speak_up('Sorry, my speech service is down')
        return voice_data


def speak_up(audio_string):
    """
...
def send_email(receiver_name, content):
...
def respond(voice_data):
    """
    handle voice data inputs and call speak_up() to convert the text responds to speech
    """
    global state
    print (voice_data)
    if 'what is your name' in voice_data:
        speak_up('My name is Google assistant')
    elif 'how are you' in voice_data:
        speak_up('I am happy to help you')
        ...
        speak_up('Here is what I found for ' + search)
    elif 'find location' in voice_data:
        location = record_audio('What is the location?')
        url = 'https://google.nl/maps/place/' + location + '/&amp;'
        webbrowser.get().open(url)
        speak_up('Here is the location of ' + location)
    elif 'what is' in voice_data:
        try:
            voice_data = voice_data.replace("what is", "")
            results = wikipedia.summary(voice_data, sentences=3)
            speak_up("According to Wikipedia, " + results)
        except:
            speak_up('Sorry. I did not get that.')
        ...
        try:
            ...
            speak_up('here are some top news in the US')
            print('''=============== UNITED STATES NEWS ============''' + '\n')
            
            for item in data['articles']:
                # print(str(i) + '. ' + item['title'] + '\n')
                speak_up(str(i) + '. ' + item['title'] + '\n')
        ...
        try:
            ...
            send_email(receiver_name, content)
            speak_up("The email has been sent!")
        except Exception as e:
            print(e)
            speak_up("I am not able to send this email")
        ...
        try:
            ...
            print(e)
            speak_up("I am not able to send this message")
    elif 'goodbye' in voice_data:
        speak_up('OK. See you later!')
        speak_up('Application change to standby mode.')
        state = 'Standby'
    elif voice_data in [ " " , "" ]:
        speak_up('Sorry. I did not get that.')
    else:
        ...
        webbrowser.get().open(url)
        speak_up('Here is what I found for ' + voice_data)
...
class MyGrid(GridLayout):
    ...
    def BtnPressed(self, instance):
        ...
        email_id = self.email.text          # Get email ID - will be changed to server email so send note to user
    ...
    def mainProcedure(self):
        ...
        state = 'Standby'
        try:
            speak_up('Start application for username: ' + username)
            porcupine = pvporcupine.create(keywords=["computer", "terminator"], sensitivities=[0.7, 0.7])
            # For DEBUG
            ...
            while state == 'Standby':
                ...
                if keyword_index == 0:
                    ...
                    state = 'Running'
                    speak_up('Hi. How can I help you?')
                if keyword_index == 1:
                    speak_up("Application Closed.")
                    state = 'Close'
                # time.sleep(0.5)
                while state == 'Running':
